...
- **Build step:** `node scripts/build-products.js` reads every file in `app/lib/raw/`,
  adds `slug`/`category`/`tagline` metadata, extracts each article's FAQ Q&A pairs,
  and generates the self-contained `app/lib/products.ts` (typed data +
  `getProductBySlug` / `getAllSlugs` helpers) plus `public/llms.txt` for AI answer
  engines. Re-run it after editing any raw article, then rebuild.
- **Routes:**
  - `/` — homepage grid linking to every review (`app/page.tsx`).
  - `/<slug>` — statically generated review page at the site root, where `<slug>` is
    the product name plus a category descriptor (e.g.
    `/mitolyn-weight-loss-supplement`), with per-page SEO metadata via
    `generateMetadata` (`app/[slug]/page.tsx`). All 14 slugs are pre-rendered through
    `generateStaticParams`. Legacy `/reviews/<slug>` URLs 308-redirect to the new
    slugs via `next.config.ts`.
